...
def create_directory_structure(base_dir, username, password):
    """
    Create the directory structure if it does not exist, including a 'files' subdirectory.
    """
    target_dir = os.path.join(base_dir, "files", username, password)
...
def is_download_needed(file_path, max_age_minutes):
    """
    Check if the file exists and if it's older than max_age_minutes.
    """
...
target_dir = create_directory_structure(config.base_dir, username, password)
